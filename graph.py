...
class Graph(object):
    ...
    def __add__(self, other: "Graph") -> "Graph":
        ...
        """
        res = Graph(self._directed, len(self) + len(other), self._simple)
        d = dict()

        for v, r in zip(self._v + other._v, res.vertices):
            d[v] = r

        for v in d:
            if hasattr(v, 'colornum'):
                d[v].colornum = v.colornum
            d[v].oldgraph = v.graph.name

        for e in self._e + other._e:
            res.add_edge(Edge(d[e.tail], d[e.head]))
    ...
    def deepcopy(self):
        """
              ...
              :return: New graph which is a disjoint union of `self' and `other'.
              """
        res = Graph(self._directed, len(self), self._simple)
        d = dict()

        for v, r in zip(self._v, res.vertices):
            d[v] = r
